(ns defensecompliance.registry
  "Pure-function filing-draft + filing-submit record construction for
  the JPN-MOD (Ministry of Defense) actor -- an append-only compliance
  book-of-record draft, one per track (`:three-principles` /
  `:specially-designated-secrets`).

  Like every sibling actor's registry, there is no single reference-
  number standard MOD assigns to a compliance filing-draft/filing-
  ...
  non-fabricating discipline `defensecompliance.facts` uses.
  ...
  call to MOD or any government portal. It builds the RECORD an
  operator would keep, not the act of actually filing/registering
  itself (that is `defensecompliance.operation`'s `:filing/submit`,
...
(defn register-draft
  ...
  (`:three-principles` or `:specially-designated-secrets`) -- the
  operator's own act of preparing a MOD compliance filing package. Pure
  function -- does not touch MOD or any government portal."
  ...
  (let [draft-number (str "JPN-MOD-" (track-code track) "-DFT-" (zero-pad sequence 6))
...
(defn register-submit
  ...
  compliance filing to MOD (always human-gated upstream)."
  [engagement-id track sequence]
  (when-not (and engagement-id (not= engagement-id ""))
  ...
  (let [submit-number (str "JPN-MOD-" (track-code track) "-SUB-" (zero-pad sequence 6))

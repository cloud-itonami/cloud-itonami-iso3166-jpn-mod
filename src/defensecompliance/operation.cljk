(ns defensecompliance.operation
  "OperationActor -- one MOD compliance operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (DefenseCompliance-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Defense-Transfer Compliance
  ...
  One graph run = one compliance operation (three-principles track or
  specially-designated-secrets track). No unbounded inner loop.
  ...
  governor is clean -- see `defensecompliance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [defensecompliance.defensecompliancellm :as defensecompliancellm]
            [defensecompliance.governor :as governor]
            [defensecompliance.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (defensecompliancellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (defensecompliancellm/-advise advisor store request)]
            {:proposal p :audit [(defensecompliancellm/trace request p)]})))

...
;;
;; JPN-MOD adaptation of cloud-itonami-iso3166-jpn-meti/scripts/verify-facts.cljs
;; (copied 2026-08-29). The verifier's reason vocabulary (:title-drift,
;; :missing-text, :charset-drift, :unexpected-redirect, :challenge-interposed,
;; :nonexistent-law, :host-soft-404, :host-status-drift, :law-title-drift,
;; :law-num-drift, etc.) is unchanged from the proven pair, because the
;; reason keywords are what scripts/mutation-check.cljs asserts by name and
;; the gate counts on the suite declaring at least 12 mutations caught by
;; their own reason. Per-ministry adaptation is in :host-behaviour entities
;; (re-measured) and in the chosen pages / law ids (re-fetched), not in the
;; verifier's reason names.
;;

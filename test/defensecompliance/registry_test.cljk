(ns defensecompliance.registry-test
...
(deftest register-draft-and-submit
  (let [d (registry/register-draft "eng-1" :three-principles 0)
        s (registry/register-submit "eng-1" :specially-designated-secrets 0)]
    (is (= "JPN-MOD-THREE-PRINCIPLES-DFT-000000" (get d "draft_number")))
    (is (= "JPN-MOD-SPECIALLY-DESIGNATED-SECRETS-SUB-000000" (get s "submit_number")))
    (is (nil? (get-in d ["certificate" "proof"])))
    (is (= "draft-unsigned" (get-in s ["certificate" "status"])))))

(deftest register-requires-ids
  (is (thrown? Exception (registry/register-draft "" :three-principles 0)))
